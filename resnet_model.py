...
def model_resnet_1d(input_shape):
    ...
    def build_resnet_1d(input_shape):
        ...
        outputs = layers.Dense(1, activation='sigmoid')(x)  # 1 output neuron
...
def model_resnet(input_shape):
    ...
    def residual_block(x, filters, kernel_size=3, stride=1):
        ...
        # Adjust the shortcut connection if the number of filters doesn't match
        if shortcut.shape[-1] != filters:
            shortcut = layers.Conv2D(filters, kernel_size=(1, 1), strides=1, padding='same')(shortcut)
            shortcut = layers.BatchNormalization()(shortcut)

        # Adding shortcut (identity mapping)
        x = layers.add([x, shortcut])
        x = layers.ReLU()(x)
    ...
    def build_resnet_model(input_shape):
        ...
        # Initial Conv layer
        x = layers.Conv2D(32, kernel_size=(3,3), strides=1, padding='same')(inputs)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
    ...
    return build_resnet_model(input_shape)

def model_resnet50_1d(input_shape):
    ...
    return build_resnet50_1d(input_shape)
...
def implement():
    ...
    #resnet_model = model_resnet_1d(input_shape)
    resnet_model = model_resnet50_1d(input_shape)
    ...
    for tester_index in range(len(validation_X)):
        ...
        predictions = resnet_model.predict(tester_data)  # X_test is your test data
        ...
        pred_label, coffidence = return_predicted_label_and_coffidence(predicted_classes)
