    ...
    for tester_id in range(NUM_TESTER):
        for question_folder in question_folders:
            ...
            if os.path.isfile(file_path):
                ...
                row_counts[tester_id] = row_count
                t_i_j[tester_id] = calculate_t_i_j(row_count)
            else:
                print(f"File not found: {file_path}")
                row_counts[tester_id] = 0  # If file does not exist, append 0
                t_i_j[tester_id] = 0  # If file does not exist, append 0
    return t_i_j
...
def find_invalid_testers(a_i_j, t_i_j, lf_j):
...
def main():
    ...
    invalid_testers = find_invalid_testers(a_i_j, t_i_j, lf_j)
